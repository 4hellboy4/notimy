...
class SecondScreenwq extends StatefulWidget {
  ...
  String userId;
  int chis;
  SecondScreenwq({Key? key, required this.NotName, required this.chis, required this.userId}) : super(key: key);
  @override
  _SecondScreenState createState() => _SecondScreenState(nameNot: NotName, ownStat: chis, usersId: userId);
}
...
class _SecondScreenState extends State<SecondScreenwq> {
  ...
  String nameNot;
  int ownStat;
  _SecondScreenState({required this.nameNot, required this.ownStat, required this.usersId});
  ...
  generationFunc() {
    ...
    db.collection('users').where('Name', isEqualTo: chislCode).get().then((value){
      var resultLeng=value.docs.length;
      if (resultLeng>0) {
        generationFunc();
      } else {
        db.collection('users').doc(currentUser!.uid).set({'Name' : chislCode},SetOptions(merge: true));
      }
    });
  }
            ...
            Padding(
              ...
              child: Container(
                ...
                child: ElevatedButton(
                  style: ElevatedButton.styleFrom(primary:const Color(0xFF00D26E)),
                  ...
                  onPressed: () async {
                    ...
                    if (hasInternet==false) {
                      ...
                      if (ownStat==1){
                        db.collection('users').doc(currentUser!.uid).set({"usersNotimy" : {'$nameNot' :{ 'status': 'loading', 'ownStatus': 1, 'id': usersId}}},SetOptions(merge: true));
                        db.collection('users').doc(currentUser!.uid).update({'creatingProcess': 0, 'whocreate': '', 'Name' : ''});
                      }else{
                        db.collection('users').doc(currentUser!.uid).set({"usersNotimy" : {'$nameNot' :{ 'status': 'loading', 'ownStatus': 0, 'id': usersId}}},SetOptions(merge: true));
                        db.collection('users').doc(currentUser!.uid).update({'creatingProcess': 0, 'whocreate': '', 'Name' : ''});
                      }
                      generationFunc();
